...
function SlackMeme(Config) 	{
	...
	function _build_message_attachments(generators, _title) {
		...
		_.each(generators, function(gen) {
			entry.push({
				title: util.format('Generator Id: %d, Name: %s', gen.id, gen.name),
				value: util.format('%s', gen.url),
				short: true
			})
		})
	...
	function _build_instance_message_attachments(instance) {
		var title = util.format('%s', instance.name)
		var results = [{
			fallback: title,
			pretext: undefined,
			color: 'good',
		...
		entry.push({
			title: title,
			value: util.format('%s\n%s\n<%s>', instance.text0, instance.text1, instance.page_url)
		})
	...
	router.post('/', function (req, res) {
		...
		async.waterfall([
			...
			var my_err = Error(util.format('imgflip.com %s failed', cmd))
		  if (err) { return res.send_err_msg(err) }

		  if (content.payload.success == "false" || content.payload.success == false || content.payload.data.length == 0) {
		  ...
		  var packet = {
		  	...
		  	username: 'memebot',
		  	text: undefined,
		  	icon_url: "http://content.superresize.me/images/ToQNg/57x57_crop_happy-oh-stop-it-you-l_1_.png",
		  	unfurl_links: true,
		  	unfurl_media: true,
		  ...
		  if (cmd == 'search') {
		  	packet.channel = util.format('@%s', req.body.user_name)
				packet.attachments = _build_message_attachments(content.payload.data.memes)
		  } else if (cmd == 'create') {
		  	var instance = content.payload.data;
		  	instance.name = content.name; 
		  	instance.text0 = content.text0; 
		  	instance.text1 = content.text1; 
		  	...
		  	packet.text = util.format('<%s>', instance.url)
